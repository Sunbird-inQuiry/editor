import { AfterViewInit, Component, ElementRef, EventEmitter, Input, OnInit , Output, ViewChild, ViewEncapsulation, OnChanges, AfterViewChecked} from '@angular/core';
  ...
  loadPlayer() {
    ...
    waitForCustomElement('sunbird-quml-player').then(() => {
      if (checkCustomElement('sunbird-quml-player') && this.inQuiryQuMLPlayer?.nativeElement) {
        ...
        this.inQuiryQuMLPlayer.nativeElement.innerHTML = ''; // Clear any existing content
